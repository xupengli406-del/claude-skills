...
- 如用户要求无照片或岗位明确要求匿名/无照片，则用上方无照片页首。缺少现成商务照时，先走 `SKILL.md` 的 Portrait preparation 分支：有清晰本人参考照就衔接内置证件照流程，没有可靠参考则引导补充照片；不要因为旧简历无照而直接跳过。只有多人照、明显文字/水印遮挡的图或身份不确定的照片时，先确认选择；不要拿身份证扫描件直接上简历。
...
- 原图完整保留；把所选图作为简历项目内相对路径依赖（如 `assets/portrait.png`），不要依赖临时剪贴板路径或易失效的远程图片链接。已有合适成片时直接复用；缺少成片时按内置证件照流程制作，严格保持本人面部特征。保留完整 3:4 成片，只在简历展示框内做必要裁切，不将照片写入 Skill 或公开仓库。
