...
公开仓库。只收录可复用且不含个人数据、账号密钥或客户隐私的 Skill。

---

### /build-hackathon-site-with-feishu — 黑客松官网与飞书管理后台

**用途**：搭建可部署的黑客松活动官网，将报名表单、选手数据、筛选状态和活动详情入口连接到飞书电子表格与飞书文档。

**默认经典基线**：先复刻双图聚光地形首屏和完整活动页结构，通过构建后再替换品牌、活动信息、嘉宾、赛道、奖项和飞书链接。

**核心能力**：

1. 黑客松活动官网、首屏动效、导航、赛道、日程、嘉宾轮播、评委、奖项和回顾
2. 报名弹窗或分步报名页，包含服务端校验、重复报名更新和状态反馈
3. 站点数据库暂存 + 飞书电子表格同步与失败重试
4. 赛事细则、选手手册、获奖信息等飞书文档入口
5. Sites 部署、移动端、无障碍、favicon 和 Open Graph

**调用**：`/build-hackathon-site-with-feishu 为这场黑客松搭建活动官网和飞书管理后台`
...
### /produce-faceless-demo-video — 无露脸录屏解说视频

**用途**：将产品录屏、网站演示、软件操作或工作流证明素材，制作成无需真人出镜、带自然中文配音与同步字幕的社媒视频。

**核心能力**：

1. 检查时长、分辨率、帧率、音轨并生成画面联系表
2. 删除报错、等待、空白、重复和误操作，按叙事节奏加速流程
3. 按“视觉钩子 → 价值 → 核心流程 → 结果证据 → 完整能力 → CTA”重组内容
4. 生成中文口播、AI 配音和 SRT 字幕
5. 输出 1920×1080、30fps 的 H.264/AAC 成片并完成解码验收
6. 检查手机号、邮箱、姓名、Token 和后台数据等隐私信息
7. 清理拆帧、临时音频和调试文件，只保留成片、口播稿和字幕

**经典案例**：83.9 秒、1280×720 的黑客松官网录屏，精简为 47.9 秒、1920×1080、30fps 的配音字幕成片。

**调用**：`/produce-faceless-demo-video 把这个产品录屏剪成一条不露脸的 60 秒解说视频`

**目录结构**：

```
produce-faceless-demo-video/
├── SKILL.md
├── agents/
│   └── openai.yaml
├── references/
│   ├── production-standard.md
│   └── segment-plan.example.json
└── scripts/
    ├── inspect-recording.ps1
    ├── new-narration.ps1
    └── render-video.ps1
```

**来源**：2026 年 7 月，从黑客松官网、报名流程与飞书后台演示视频的完整制作闭环中提炼。
